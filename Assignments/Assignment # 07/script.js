...
// Function to count minimum number of notes
function countNotes(amount) {
    const denominations = [5000, 1000, 500, 100, 50, 20, 10, 5, 1];
    const notesCount = {};
    for (let i = 0; i < denominations.length; i++) {
        const count = Math.floor(amount / denominations[i]);
        notesCount[denominations[i]] = count;
        amount -= count * denominations[i];
    }

    return notesCount;
}
const amount = parseInt(prompt("Enter the amount in PKR:"));
const notes = countNotes(amount);

let message = "Minimum number of notes:\n";
for (const denomination in notes) {
    if (notes[denomination] !== 0) {
        message += `${notes[denomination]} notes of Rs.${denomination}\n`;
    }
}
alert(message);
